...
export default function AddressPage() {
  ...
  // Limpa o erro ao focar de volta no campo CEP
  const handleCepFocus = () => {
    setHasError(false);
  };

  // Executa a validação e busca ao sair do campo CEP
  const handleCepBlur = async () => {
    const cleanCep = formData.cep.replace(/\D/g, ''); // Remove traços se houver
  ...
  return (
    <main>
      <form className={isLoading ? 'loading' : ''} onSubmit={(e) => e.preventDefault()}>
        <h2>Address</h2>
